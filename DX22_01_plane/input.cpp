...
void Input::Update() {
    ...
    ScreenToClient(Application::GetWindow(), &mouseposition);
...
DirectX::XMFLOAT2 Input::GetMousePosition() {
    ...
    // 原点は画面中央、Y軸は上方向がプラス
    const float x =
        m_MousePosition.x * Application::GetWidth() / clientWidth
        - Application::GetWidth() * 0.5f;

    const float y =
        Application::GetHeight() * 0.5f
        - m_MousePosition.y * Application::GetHeight() / clientHeight;
